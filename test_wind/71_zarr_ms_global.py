...
def run_ms_global(l):
    ...
    if l==labels[0] : 
        ms_dataset(dsm, l).to_zarr(os.path.join(zarr_dir+'_ok','global_minus',f'ms_{int(DL//1000)}_{DT}_minus.zarr'),encoding={'drifter_sat_year':{'dtype':'U32'}}, mode='w')
        mean_dataset(dsm, l).to_zarr(os.path.join(zarr_dir+'_ok','global_minus',f'mean_{int(DL//1000)}_{DT}_minus.zarr'),encoding={'drifter_sat_year':{'dtype':'U32'}}, mode='w')
        if dsmd.dims['obs']!=0 : 
            ms_dataset(dsmd,l).to_zarr(os.path.join(zarr_dir+'_ok','global_minus',f'ms_{int(DL//1000)}_{DT}_drogued_minus.zarr'), encoding={'drifter_sat_year':{'dtype':'U32'}},mode='w')
            mean_dataset(dsmd,l).to_zarr(os.path.join(zarr_dir+'_ok','global_minus',f'mean_{int(DL//1000)}_{DT}_drogued_minus.zarr'), encoding={'drifter_sat_year':{'dtype':'U32'}},mode='w')
        if dsmnd.dims['obs']!=0 : 
            ms_dataset(dsmnd,l).to_zarr(os.path.join(zarr_dir+'_ok','global_minus',f'ms_{int(DL//1000)}_{DT}_undrogued_minus.zarr'),encoding={'drifter_sat_year':{'dtype':'U32'}}, mode='w')
            mean_dataset(dsmnd,l).to_zarr(os.path.join(zarr_dir+'_ok','global_minus',f'mean_{int(DL//1000)}_{DT}_undrogued_minus.zarr'),encoding={'drifter_sat_year':{'dtype':'U32'}}, mode='w')
    else : 
        ms_dataset(dsm,l).to_zarr(os.path.join(zarr_dir+'_ok','global_minus',f'ms_{int(DL//1000)}_{DT}_minus.zarr'), append_dim='drifter_sat_year')
        mean_dataset(dsm,l).to_zarr(os.path.join(zarr_dir+'_ok','global_minus',f'mean_{int(DL//1000)}_{DT}_minus.zarr'), append_dim='drifter_sat_year')
        if dsmd.dims['obs']!=0 : 
            ms_dataset(dsmd,l).to_zarr(os.path.join(zarr_dir+'_ok','global_minus',f'ms_{int(DL//1000)}_{DT}_drogued_minus.zarr'), append_dim='drifter_sat_year')
            mean_dataset(dsmd,l).to_zarr(os.path.join(zarr_dir+'_ok','global_minus',f'mean_{int(DL//1000)}_{DT}_drogued_minus.zarr'), append_dim='drifter_sat_year')
        if dsmnd.dims['obs']!=0 : 
            ms_dataset(dsmnd,l).to_zarr(os.path.join(zarr_dir+'_ok','global_minus',f'ms_{int(DL//1000)}_{DT}_undrogued_minus.zarr'),append_dim='drifter_sat_year')
            mean_dataset(dsmnd,l).to_zarr(os.path.join(zarr_dir+'_ok','global_minus',f'mean_{int(DL//1000)}_{DT}_undrogued_minus.zarr'),append_dim='drifter_sat_year')
